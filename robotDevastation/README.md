...
## The Game FSM
The game functionality is implemented as a [Finite State Machine (FSM)](https://en.wikipedia.org/wiki/Finite-state_machine), which will be explained in the [Game FSM](game-fsm/README.md) section. Each state of the FSM represents a game state. Each game state inherits from the [ManagerHub class](https://asrob.uc3m.es/robotDevastation/classrd_1_1ManagerHub.html), which allows the state to act upon the different game subsystems (user input, user interface, robot, etc).
